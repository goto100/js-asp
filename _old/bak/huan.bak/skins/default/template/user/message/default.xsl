...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/messages/item">
		<xsl:element name="tr" use-attribute-sets="listTable.row">
			<td><a href="?id={sender/@id}"><xsl:value-of select="sender" /></a></td>
	...
	<xsl:template match="/sentMessages/item">
		<xsl:element name="tr" use-attribute-sets="listTable.row">
			<td><a href="?id={reciver/@id}"><xsl:value-of select="reciver" /></a></td>
			<th scope="row"><a href="?id={@id}"><xsl:value-of select="title" /></a></th>
			<td><xsl:value-of select="sendTime" /></td>
			<td><a href="?delete&amp;id={@id}">删除</a></td>
		</xsl:element>
	...
	<xsl:template match="/message">
		<div class="info">
			<ul>
				<li>发件人：<a href="?id={sender/@id}"><xsl:value-of select="sender" /></a></li>
				<li>收件人：<a href="?id={reciver/@id}"><xsl:value-of select="reciver" /></a></li>
