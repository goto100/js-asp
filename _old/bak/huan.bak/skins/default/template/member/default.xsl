...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<xsl:choose>
			...
			<xsl:when test="user">
				<xsl:apply-templates select="." mode="default">
					<xsl:with-param name="title"><xsl:value-of select="user/nickname" /></xsl:with-param>
				</xsl:apply-templates>
			</xsl:when>
	...
	<xsl:template match="/users/item">
		<xsl:element name="tr" use-attribute-sets="listTable.row">
			<th scope="row"><a href="../../../../skins/default/template/user/?id={@id}"><xsl:value-of select="userId" /></a></th>
