...
function HuanSetting(system) {
	...
	this.load = function() {
		var record = system.db.query("SELECT * FROM " + system.db.SETTINGS, 1);
		this.fill(record);
	}

	this.getVbArr = function() {
		return system.db.query("SELECT siteName, siteURL, siteOpened, defaultLanguage, defaultSkin, defaultStyle, recordUsersTimeOut, recordVisitor FROM " + system.db.SETTINGS, 1, null, true);
	}

	this.update = function(value) {
		if (value) this.fill(value);

		system.db.update(system.db.SETTINGS, {
