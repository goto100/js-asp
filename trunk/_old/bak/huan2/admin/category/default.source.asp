...
function Main() {
	...
	actUp.action = function() {
		...
		category.load();
		category.up()? actTree.execute("categoryUped") : actTree.execute("categoryAlreadyTop");
	}

	actDown.action = function() {
		var category = system.getCategory(this.getIdParam());
		category.load();
		category.down()? actTree.execute("categoryDowned") : actTree.execute("categoryAlreadyBottom");
	}
	...
	actDelete.action = function() {
		...
		category.load();
		category.del();
		actTree.execute("categoryDeleted");
	}
	...
	actUpdate.action = function() {
		...
		this.withStringParam("url", false, 10);
