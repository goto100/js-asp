...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="category" mode="category.item">
		...
		<li><xsl:value-of select="$name" />
			<span class="add"><a href="?add&amp;parentId={$id}" title="添加">添加</a></span>
			<xsl:if test="not($isFirst)">
				<span class="up"><a href="?up&amp;id={$id}" title="向上">向上</a></span>
			</xsl:if>
			<xsl:if test="not($isLast)">
				<span class="down"><a href="?down&amp;id={$id}" title="向下">向下</a></span>
			</xsl:if>
			<span class="edit"><a href="?edit&amp;id={$id}" title="编辑">编辑</a></span>
			<xsl:if test="$isLeaf"><span class="del"><a href="?delete&amp;id={$id}" title="删除">删除</a></span></xsl:if>
