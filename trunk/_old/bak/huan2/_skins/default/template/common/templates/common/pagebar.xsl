		...
		<div class="pagebar">
			<xsl:choose>
				<xsl:when test="$currentPage &gt; 1">
					<p class="previous"><a href="?page={$currentPage - 1}" title="上一页">&lt;</a></p>
				</xsl:when>
				<xsl:otherwise>
					<p class="previous">&lt;</p>
				</xsl:otherwise>
			...
			<xsl:choose>
				<xsl:when test="$currentPage &lt; $pageCount">
					<p class="next"><a href="?page={$currentPage + 1}" title="下一页">&gt;</a></p>
				</xsl:when>
				<xsl:otherwise>
					<p class="next">&gt;</p>
				</xsl:otherwise>
			...
			<ul>
				<xsl:if test="$start &gt; 1">
					<li class="first"><a href="?page=1" title="首页">&lt;&lt;</a></li>
				</xsl:if>
				...
				<xsl:if test="$start + $count &lt; $pageCount + 1">
					<li class="last"><a href="?page={$pageCount}" title="末页">&gt;&gt;</a></li>
				</xsl:if>
			</ul>
	...
	<xsl:template match="*" mode="pagebar.item">
		...
		<xsl:if test="$total > 0">
			<xsl:choose>
				...
				<xsl:otherwise>
					<li class="item"><a href="?page={$start}" title="第 {$start} 页"><xsl:value-of select="$start" /></a></li>
				</xsl:otherwise>
			</xsl:choose>
			<xsl:apply-templates select="." mode="pagebar.item">
