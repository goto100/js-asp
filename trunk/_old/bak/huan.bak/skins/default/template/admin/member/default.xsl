...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
			...
			<xsl:when test="users">
				<xsl:apply-templates select="." mode="default">
					<xsl:with-param name="title">用户列表
						<xsl:if test="users/@keys"> - <xsl:value-of select="users/@keys" /></xsl:if>
					</xsl:with-param>
					<xsl:with-param name="sidebar">
						<xsl:call-template name="adminPanel" />
						<xsl:apply-templates select="/users/order" mode="orderPanel" />
						<div class="panel">
							<h2>搜索用户</h2>
							<form action=".">
								<input type="text" name="keys" value="{users/@keys}" size="12" />
	...
	<xsl:template match="/users/order/userId">用户名</xsl:template>
	<xsl:template match="/users/order/regTime">注册时间</xsl:template>
	<xsl:template match="/users/order/nickname">昵称</xsl:template>
	<xsl:template match="/users/order/lastVisitTime">最后访问时间</xsl:template>
	...
	<xsl:template match="/add/user | /edit/user">
		<fieldset>
			<legend>基本信息</legend>
	...
	<xsl:template match="/add/userGroups/userGroup | /edit/userGroups/userGroup">
		<xsl:apply-templates select="." mode="form.option">
			...
			<xsl:with-param name="selected" select="../../../*/user/groupId = @id" />
		</xsl:apply-templates>
	</xsl:template>
