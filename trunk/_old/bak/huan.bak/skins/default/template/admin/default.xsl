...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
		...
		<xsl:choose>
			<xsl:when test="systemInfo">
				<xsl:apply-templates select="." mode="default">
					<xsl:with-param name="title">Huan System - 系统信息</xsl:with-param>
	...
	<xsl:template match="/systemInfo">
		<div id="systemInfo">
			<p><a href="?reloadcache">刷新缓存</a></p>
			<p><a href="?buildcache">重建缓存文件</a></p>
			<p>当前时间：<xsl:value-of select="now" /></p>
			<p>系统版本：<xsl:value-of select="version" /></p>
			<p>更新日期：<xsl:value-of select="updateDate" /></p>
			<p>当前在线：<xsl:value-of select="visitorCount" /></p>
			<p>注册会员：<xsl:value-of select="userCount" /></p>
			<p>访问次数：<xsl:value-of select="visitorCount" /></p>
		</div>
	</xsl:template>
